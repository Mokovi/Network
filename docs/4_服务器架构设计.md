# 服务器架构设计

## 1. 文档概述

本文档详细介绍了Linux网络编程中的服务器架构设计，包括基础服务器模型、高级服务器模型、事件驱动架构、负载均衡与代理等核心内容。服务器架构设计是构建高性能、高可用网络应用的关键技术，掌握这些知识对于开发企业级网络服务至关重要。

本文档将从简单的串行服务器开始，逐步深入到多进程、多线程、事件驱动等复杂架构，帮助读者理解不同架构的优缺点、适用场景以及实现方法。通过学习本文档，读者将能够：

- 理解各种服务器架构模型的特点和适用场景
- 掌握多进程和多线程服务器的实现方法
- 理解Reactor和Proactor事件驱动模式
- 学会设计高性能的服务器架构
- 了解负载均衡和反向代理的实现原理

## 2. 服务器架构基础

### 2.1 服务器架构分类

服务器架构可以根据处理连接的方式进行分类：

```
┌─────────────────────────────────────────────────────────────┐
│                    服务器架构分类体系                          │
├─────────────────────────────────────────────────────────────┤
│  📋 基础架构模型                                              │
│  ├── 串行/迭代服务器 (Serial/Iterative Server)              │
│  ├── 进程每连接模型 (Process-per-Connection)                │
│  └── 线程每连接模型 (Thread-per-Connection)                 │
├─────────────────────────────────────────────────────────────┤
│  ⚡ 高级架构模型                                              │
│  ├── 线程池模型 (Thread Pool)                               │
│  ├── 进程池模型 (Process Pool / Prefork)                   │
│  ├── 事件驱动模型 (Event-Driven)                            │
│  │   ├── Reactor模式                                        │
│  │   └── Proactor模式                                       │
│  └── 混合架构模型 (Hybrid Architecture)                     │
├─────────────────────────────────────────────────────────────┤
│  🏢 分布式架构                                               │
│  ├── 负载均衡 (Load Balancing)                              │
│  ├── 反向代理 (Reverse Proxy)                               │
│  └── 服务发现 (Service Discovery)                           │
└─────────────────────────────────────────────────────────────┘
```

### 2.2 架构选择因素

在选择服务器架构时，需要考虑以下因素：

| 因素 | 说明 | 影响 |
|------|------|------|
| **并发连接数** | 同时处理的连接数量 | 决定是否需要多进程/多线程 |
| **请求处理时间** | 单个请求的处理耗时 | 影响是否需要异步处理 |
| **I/O类型** | I/O密集型 vs CPU密集型 | 决定使用事件驱动还是线程池 |
| **资源限制** | 内存、CPU、文件描述符限制 | 影响进程/线程数量 |
| **可扩展性** | 水平扩展 vs 垂直扩展 | 影响架构设计 |
| **开发复杂度** | 代码复杂度和维护成本 | 影响架构选择 |

### 2.3 性能指标

评估服务器架构性能的主要指标：

**吞吐量（Throughput）：**
- 单位时间内处理的请求数（QPS - Queries Per Second）
- 单位时间内传输的数据量（BPS - Bytes Per Second）

**延迟（Latency）：**
- 请求处理时间（P99延迟、平均延迟）
- 连接建立时间

**并发能力（Concurrency）：**
- 最大并发连接数
- 资源利用率（CPU、内存、网络）

**可扩展性（Scalability）：**
- 水平扩展能力
- 垂直扩展能力

## 3. 基础服务器模型

### 3.1 串行/迭代服务器

串行服务器是最简单的服务器模型，每次只处理一个客户端连接，处理完当前连接后才能接受下一个连接。

#### 3.1.1 串行服务器特点

**优点：**
- 实现简单，易于理解
- 不需要考虑并发问题
- 资源占用少
- 适合简单的网络应用

**缺点：**
- 性能极差，无法并发处理
- 一个客户端阻塞会影响所有后续客户端
- 不适合生产环境

**适用场景：**
- 学习网络编程的入门示例
- 简单的工具程序
- 低并发的内部服务

#### 3.1.2 串行服务器实现

```c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
#include <sys/socket.h>
#include <netinet/in.h>
#include <arpa/inet.h>

#define PORT 8080
#define BUFFER_SIZE 1024

void handle_client(int client_fd) {
    char buffer[BUFFER_SIZE];
    ssize_t bytes_received;
    
    printf("Handling client connection\n");
    
    while (1) {
        // 接收数据
        bytes_received = recv(client_fd, buffer, BUFFER_SIZE - 1, 0);
        if (bytes_received <= 0) {
            break;
        }
        
        buffer[bytes_received] = '\0';
        printf("Received: %s", buffer);
        
        // 处理请求（模拟处理时间）
        // 这里可以添加业务逻辑处理
        
        // 发送响应
        send(client_fd, buffer, bytes_received, 0);
    }
    
    close(client_fd);
    printf("Client disconnected\n");
}

int main() {
    int server_fd, client_fd;
    struct sockaddr_in server_addr, client_addr;
    socklen_t client_len;
    
    // 创建套接字
    server_fd = socket(AF_INET, SOCK_STREAM, 0);
    if (server_fd == -1) {
        perror("socket");
        exit(EXIT_FAILURE);
    }
    
    // 设置地址重用
    int opt = 1;
    setsockopt(server_fd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt));
    
    // 配置服务器地址
    memset(&server_addr, 0, sizeof(server_addr));
    server_addr.sin_family = AF_INET;
    server_addr.sin_addr.s_addr = INADDR_ANY;
    server_addr.sin_port = htons(PORT);
    
    // 绑定地址
    if (bind(server_fd, (struct sockaddr*)&server_addr, 
             sizeof(server_addr)) == -1) {
        perror("bind");
        close(server_fd);
        exit(EXIT_FAILURE);
    }
    
    // 开始监听
    if (listen(server_fd, 5) == -1) {
        perror("listen");
        close(server_fd);
        exit(EXIT_FAILURE);
    }
    
    printf("Serial Server listening on port %d\n", PORT);
    printf("Warning: This server can only handle one client at a time!\n");
    
    // 串行处理连接
    while (1) {
        client_len = sizeof(client_addr);
        client_fd = accept(server_fd, (struct sockaddr*)&client_addr, 
                          &client_len);
        if (client_fd == -1) {
            perror("accept");
            continue;
        }
        
        printf("Client connected from %s:%d\n",
               inet_ntoa(client_addr.sin_addr),
               ntohs(client_addr.sin_port));
        
        // 处理客户端请求（阻塞）
        handle_client(client_fd);
        
        // 只有处理完当前客户端，才能接受下一个连接
    }
    
    close(server_fd);
    return 0;
}
```

#### 3.1.3 串行服务器性能分析

| 指标 | 值 | 说明 |
|------|-----|------|
| **并发连接数** | 1 | 同时只能处理一个连接 |
| **吞吐量** | 低 | 受限于单线程处理 |
| **延迟** | 高 | 后续客户端需要等待 |
| **资源占用** | 低 | 单进程单线程 |
| **适用场景** | 学习/演示 | 不适合生产环境 |

### 3.2 进程每连接模型

进程每连接模型（Process-per-Connection）为每个客户端连接创建一个独立的子进程来处理请求。

#### 3.2.1 进程每连接模型特点

**优点：**
- 进程间隔离，一个进程崩溃不影响其他进程
- 编程相对简单，不需要考虑线程同步
- 可以利用多核CPU
- 适合CPU密集型任务

**缺点：**
- 进程创建和销毁开销大
- 内存占用高（每个进程独立地址空间）
- 进程间通信复杂
- 不适合高并发场景

**适用场景：**
- 中等并发（< 1000连接）
- CPU密集型任务
- 需要进程隔离的场景

#### 3.2.2 进程每连接模型实现

```c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
#include <sys/socket.h>
#include <netinet/in.h>
#include <arpa/inet.h>
#include <sys/wait.h>
#include <signal.h>

#define PORT 8080
#define BUFFER_SIZE 1024

// 信号处理：回收僵尸进程
void sigchld_handler(int sig) {
    while (waitpid(-1, NULL, WNOHANG) > 0);
}

void handle_client(int client_fd) {
    char buffer[BUFFER_SIZE];
    ssize_t bytes_received;
    
    printf("[PID %d] Handling client connection\n", getpid());
    
    while (1) {
        bytes_received = recv(client_fd, buffer, BUFFER_SIZE - 1, 0);
        if (bytes_received <= 0) {
            break;
        }
        
        buffer[bytes_received] = '\0';
        printf("[PID %d] Received: %s", getpid(), buffer);
        
        // 处理请求
        // 这里可以添加业务逻辑
        
        // 发送响应
        send(client_fd, buffer, bytes_received, 0);
    }
    
    close(client_fd);
    printf("[PID %d] Client disconnected, process exiting\n", getpid());
    exit(0);
}

int main() {
    int server_fd, client_fd;
    struct sockaddr_in server_addr, client_addr;
    socklen_t client_len;
    pid_t child_pid;
    
    // 注册信号处理函数
    signal(SIGCHLD, sigchld_handler);
    
    // 创建套接字
    server_fd = socket(AF_INET, SOCK_STREAM, 0);
    if (server_fd == -1) {
        perror("socket");
        exit(EXIT_FAILURE);
    }
    
    // 设置地址重用
    int opt = 1;
    setsockopt(server_fd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt));
    
    // 配置服务器地址
    memset(&server_addr, 0, sizeof(server_addr));
    server_addr.sin_family = AF_INET;
    server_addr.sin_addr.s_addr = INADDR_ANY;
    server_addr.sin_port = htons(PORT);
    
    // 绑定地址
    if (bind(server_fd, (struct sockaddr*)&server_addr, 
             sizeof(server_addr)) == -1) {
        perror("bind");
        close(server_fd);
        exit(EXIT_FAILURE);
    }
    
    // 开始监听
    if (listen(server_fd, 5) == -1) {
        perror("listen");
        close(server_fd);
        exit(EXIT_FAILURE);
    }
    
    printf("Process-per-Connection Server listening on port %d\n", PORT);
    printf("Main process PID: %d\n", getpid());
    
    while (1) {
        client_len = sizeof(client_addr);
        client_fd = accept(server_fd, (struct sockaddr*)&client_addr, 
                          &client_len);
        if (client_fd == -1) {
            perror("accept");
            continue;
        }
        
        printf("Client connected from %s:%d\n",
               inet_ntoa(client_addr.sin_addr),
               ntohs(client_addr.sin_port));
        
        // 创建子进程处理客户端
        child_pid = fork();
        if (child_pid == -1) {
            perror("fork");
            close(client_fd);
            continue;
        }
        
        if (child_pid == 0) {
            // 子进程：关闭监听套接字，处理客户端
            close(server_fd);
            handle_client(client_fd);
            // handle_client返回后会exit
        } else {
            // 父进程：关闭客户端套接字，继续监听
            close(client_fd);
            printf("Created child process %d for client\n", child_pid);
        }
    }
    
    close(server_fd);
    return 0;
}
```

#### 3.2.3 进程每连接模型优化

**预先fork模型（Prefork）：**
预先创建一定数量的子进程，避免频繁fork的开销：

```c
#define PREFORK_COUNT 10

void prefork_workers(int server_fd) {
    pid_t pid;
    int i;
    
    for (i = 0; i < PREFORK_COUNT; i++) {
        pid = fork();
        if (pid == 0) {
            // 子进程：处理连接
            worker_process(server_fd);
            exit(0);
        } else if (pid < 0) {
            perror("fork");
            exit(EXIT_FAILURE);
        }
        printf("Created worker process %d\n", pid);
    }
}

void worker_process(int server_fd) {
    int client_fd;
    struct sockaddr_in client_addr;
    socklen_t client_len;
    
    while (1) {
        client_len = sizeof(client_addr);
        client_fd = accept(server_fd, (struct sockaddr*)&client_addr, 
                          &client_len);
        if (client_fd == -1) {
            perror("accept");
            continue;
        }
        
        printf("[Worker %d] Handling client\n", getpid());
        handle_client(client_fd);
        close(client_fd);
    }
}
```

#### 3.2.4 进程每连接模型性能分析

| 指标 | 值 | 说明 |
|------|-----|------|
| **并发连接数** | 中等（< 1000） | 受限于进程数量 |
| **吞吐量** | 中等 | 进程切换开销 |
| **延迟** | 中等 | 进程创建开销 |
| **资源占用** | 高 | 每个进程独立内存空间 |
| **适用场景** | 中等并发、CPU密集型 | Apache prefork模式 |

### 3.3 线程每连接模型

线程每连接模型（Thread-per-Connection）为每个客户端连接创建一个独立的线程来处理请求。

#### 3.3.1 线程每连接模型特点

**优点：**
- 线程创建和销毁开销比进程小
- 线程间共享内存空间，通信方便
- 可以充分利用多核CPU
- 适合I/O密集型任务

**缺点：**
- 线程创建仍有开销
- 需要处理线程同步问题
- 线程数量受限于系统资源
- 线程切换开销

**适用场景：**
- 中等并发（< 5000连接）
- I/O密集型任务
- 需要共享数据的场景

#### 3.3.2 线程每连接模型实现

```c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
#include <pthread.h>
#include <sys/socket.h>
#include <netinet/in.h>
#include <arpa/inet.h>

#define PORT 8080
#define BUFFER_SIZE 1024

// 客户端信息结构
typedef struct {
    int client_fd;
    struct sockaddr_in client_addr;
} client_info_t;

void* handle_client(void* arg) {
    client_info_t* info = (client_info_t*)arg;
    int client_fd = info->client_fd;
    char buffer[BUFFER_SIZE];
    ssize_t bytes_received;
    
    printf("[Thread %lu] Handling client from %s:%d\n",
           pthread_self(),
           inet_ntoa(info->client_addr.sin_addr),
           ntohs(info->client_addr.sin_port));
    
    while (1) {
        bytes_received = recv(client_fd, buffer, BUFFER_SIZE - 1, 0);
        if (bytes_received <= 0) {
            break;
        }
        
        buffer[bytes_received] = '\0';
        printf("[Thread %lu] Received: %s", pthread_self(), buffer);
        
        // 处理请求
        // 这里可以添加业务逻辑
        
        // 发送响应
        send(client_fd, buffer, bytes_received, 0);
    }
    
    close(client_fd);
    printf("[Thread %lu] Client disconnected, thread exiting\n", pthread_self());
    
    free(info);
    return NULL;
}

int main() {
    int server_fd, client_fd;
    struct sockaddr_in server_addr, client_addr;
    socklen_t client_len;
    pthread_t thread_id;
    client_info_t* client_info;
    
    // 创建套接字
    server_fd = socket(AF_INET, SOCK_STREAM, 0);
    if (server_fd == -1) {
        perror("socket");
        exit(EXIT_FAILURE);
    }
    
    // 设置地址重用
    int opt = 1;
    setsockopt(server_fd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt));
    
    // 配置服务器地址
    memset(&server_addr, 0, sizeof(server_addr));
    server_addr.sin_family = AF_INET;
    server_addr.sin_addr.s_addr = INADDR_ANY;
    server_addr.sin_port = htons(PORT);
    
    // 绑定地址
    if (bind(server_fd, (struct sockaddr*)&server_addr, 
             sizeof(server_addr)) == -1) {
        perror("bind");
        close(server_fd);
        exit(EXIT_FAILURE);
    }
    
    // 开始监听
    if (listen(server_fd, 5) == -1) {
        perror("listen");
        close(server_fd);
        exit(EXIT_FAILURE);
    }
    
    printf("Thread-per-Connection Server listening on port %d\n", PORT);
    
    while (1) {
        client_len = sizeof(client_addr);
        client_fd = accept(server_fd, (struct sockaddr*)&client_addr, 
                          &client_len);
        if (client_fd == -1) {
            perror("accept");
            continue;
        }
        
        // 分配客户端信息结构
        client_info = malloc(sizeof(client_info_t));
        if (client_info == NULL) {
            perror("malloc");
            close(client_fd);
            continue;
        }
        
        client_info->client_fd = client_fd;
        client_info->client_addr = client_addr;
        
        // 创建线程处理客户端
        if (pthread_create(&thread_id, NULL, handle_client, client_info) != 0) {
            perror("pthread_create");
            free(client_info);
            close(client_fd);
            continue;
        }
        
        // 分离线程，使其自动回收资源
        pthread_detach(thread_id);
        printf("Created thread %lu for client\n", thread_id);
    }
    
    close(server_fd);
    return 0;
}
```

#### 3.3.3 线程每连接模型优化

**线程属性设置：**
```c
#include <pthread.h>

// 设置线程为分离状态
pthread_attr_t attr;
pthread_attr_init(&attr);
pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED);

// 创建线程
pthread_create(&thread_id, &attr, handle_client, client_info);
pthread_attr_destroy(&attr);
```

**线程栈大小调整：**
```c
// 设置线程栈大小（默认可能较大，可以减小以节省内存）
size_t stack_size = 64 * 1024;  // 64KB
pthread_attr_setstacksize(&attr, stack_size);
```

#### 3.3.4 线程每连接模型性能分析

| 指标 | 值 | 说明 |
|------|-----|------|
| **并发连接数** | 中等（< 5000） | 受限于线程数量 |
| **吞吐量** | 中等-高 | 线程切换开销较小 |
| **延迟** | 低-中等 | 线程创建开销较小 |
| **资源占用** | 中等 | 线程共享地址空间 |
| **适用场景** | 中等并发、I/O密集型 | Java传统服务器模型 |

## 4. 高级服务器模型

### 4.1 线程池模型

线程池模型（Thread Pool）预先创建一定数量的工作线程，使用任务队列管理客户端连接，避免了频繁创建和销毁线程的开销。

#### 4.1.1 线程池模型特点

**优点：**
- 避免线程创建和销毁的开销
- 控制并发线程数量，避免资源耗尽
- 提高响应速度（线程已就绪）
- 适合高并发场景

**缺点：**
- 实现复杂度较高
- 需要任务队列和同步机制
- 线程数量需要合理配置

**适用场景：**
- 高并发场景（> 1000连接）
- I/O密集型任务
- 需要控制资源使用的场景

#### 4.1.2 线程池模型实现

```c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
#include <pthread.h>
#include <sys/socket.h>
#include <netinet/in.h>
#include <arpa/inet.h>
#include <errno.h>

#define PORT 8080
#define BUFFER_SIZE 1024
#define THREAD_POOL_SIZE 10
#define MAX_QUEUE_SIZE 100

// 任务结构
typedef struct task {
    int client_fd;
    struct sockaddr_in client_addr;
    struct task* next;
} task_t;

// 线程池结构
typedef struct {
    pthread_t* threads;
    int thread_count;
    task_t* task_queue_head;
    task_t* task_queue_tail;
    pthread_mutex_t queue_mutex;
    pthread_cond_t queue_cond;
    int shutdown;
} thread_pool_t;

thread_pool_t* pool;

// 处理客户端请求
void handle_client(int client_fd, struct sockaddr_in* client_addr) {
    char buffer[BUFFER_SIZE];
    ssize_t bytes_received;
    
    printf("[Thread %lu] Handling client from %s:%d\n",
           pthread_self(),
           inet_ntoa(client_addr->sin_addr),
           ntohs(client_addr->sin_port));
    
    while (1) {
        bytes_received = recv(client_fd, buffer, BUFFER_SIZE - 1, 0);
        if (bytes_received <= 0) {
            break;
        }
        
        buffer[bytes_received] = '\0';
        printf("[Thread %lu] Received: %s", pthread_self(), buffer);
        
        // 处理请求
        // 这里可以添加业务逻辑
        
        // 发送响应
        send(client_fd, buffer, bytes_received, 0);
    }
    
    close(client_fd);
    printf("[Thread %lu] Client disconnected\n", pthread_self());
}

// 工作线程函数
void* worker_thread(void* arg) {
    task_t* task;
    
    while (1) {
        // 获取任务
        pthread_mutex_lock(&pool->queue_mutex);
        
        // 等待任务或关闭信号
        while (pool->task_queue_head == NULL && !pool->shutdown) {
            pthread_cond_wait(&pool->queue_cond, &pool->queue_mutex);
        }
        
        // 检查是否关闭
        if (pool->shutdown) {
            pthread_mutex_unlock(&pool->queue_mutex);
            break;
        }
        
        // 从队列取出任务
        task = pool->task_queue_head;
        if (task != NULL) {
            pool->task_queue_head = task->next;
            if (pool->task_queue_head == NULL) {
                pool->task_queue_tail = NULL;
            }
        }
        
        pthread_mutex_unlock(&pool->queue_mutex);
        
        // 处理任务
        if (task != NULL) {
            handle_client(task->client_fd, &task->client_addr);
            free(task);
        }
    }
    
    return NULL;
}

// 添加任务到队列
int add_task(int client_fd, struct sockaddr_in* client_addr) {
    task_t* task = malloc(sizeof(task_t));
    if (task == NULL) {
        return -1;
    }
    
    task->client_fd = client_fd;
    task->client_addr = *client_addr;
    task->next = NULL;
    
    pthread_mutex_lock(&pool->queue_mutex);
    
    // 检查队列是否已满
    // 这里简化处理，实际应该检查队列大小
    
    if (pool->task_queue_tail == NULL) {
        pool->task_queue_head = task;
        pool->task_queue_tail = task;
    } else {
        pool->task_queue_tail->next = task;
        pool->task_queue_tail = task;
    }
    
    pthread_cond_signal(&pool->queue_cond);
    pthread_mutex_unlock(&pool->queue_mutex);
    
    return 0;
}

// 初始化线程池
thread_pool_t* thread_pool_init(int thread_count) {
    int i;
    thread_pool_t* pool = malloc(sizeof(thread_pool_t));
    if (pool == NULL) {
        return NULL;
    }
    
    pool->threads = malloc(sizeof(pthread_t) * thread_count);
    if (pool->threads == NULL) {
        free(pool);
        return NULL;
    }
    
    pool->thread_count = thread_count;
    pool->task_queue_head = NULL;
    pool->task_queue_tail = NULL;
    pool->shutdown = 0;
    
    pthread_mutex_init(&pool->queue_mutex, NULL);
    pthread_cond_init(&pool->queue_cond, NULL);
    
    // 创建工作线程
    for (i = 0; i < thread_count; i++) {
        if (pthread_create(&pool->threads[i], NULL, worker_thread, NULL) != 0) {
            // 创建失败，清理资源
            pool->shutdown = 1;
            pthread_cond_broadcast(&pool->queue_cond);
            for (int j = 0; j < i; j++) {
                pthread_join(pool->threads[j], NULL);
            }
            free(pool->threads);
            pthread_mutex_destroy(&pool->queue_mutex);
            pthread_cond_destroy(&pool->queue_cond);
            free(pool);
            return NULL;
        }
    }
    
    return pool;
}

// 销毁线程池
void thread_pool_destroy(thread_pool_t* pool) {
    int i;
    
    if (pool == NULL) {
        return;
    }
    
    // 设置关闭标志
    pthread_mutex_lock(&pool->queue_mutex);
    pool->shutdown = 1;
    pthread_mutex_unlock(&pool->queue_mutex);
    
    // 唤醒所有等待的线程
    pthread_cond_broadcast(&pool->queue_cond);
    
    // 等待所有线程退出
    for (i = 0; i < pool->thread_count; i++) {
        pthread_join(pool->threads[i], NULL);
    }
    
    // 清理剩余任务
    task_t* task = pool->task_queue_head;
    while (task != NULL) {
        task_t* next = task->next;
        close(task->client_fd);
        free(task);
        task = next;
    }
    
    // 销毁同步对象
    pthread_mutex_destroy(&pool->queue_mutex);
    pthread_cond_destroy(&pool->queue_cond);
    
    // 释放资源
    free(pool->threads);
    free(pool);
}

int main() {
    int server_fd, client_fd;
    struct sockaddr_in server_addr, client_addr;
    socklen_t client_len;
    
    // 初始化线程池
    pool = thread_pool_init(THREAD_POOL_SIZE);
    if (pool == NULL) {
        fprintf(stderr, "Failed to create thread pool\n");
        exit(EXIT_FAILURE);
    }
    
    printf("Thread Pool initialized with %d threads\n", THREAD_POOL_SIZE);
    
    // 创建套接字
    server_fd = socket(AF_INET, SOCK_STREAM, 0);
    if (server_fd == -1) {
        perror("socket");
        thread_pool_destroy(pool);
        exit(EXIT_FAILURE);
    }
    
    // 设置地址重用
    int opt = 1;
    setsockopt(server_fd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt));
    
    // 配置服务器地址
    memset(&server_addr, 0, sizeof(server_addr));
    server_addr.sin_family = AF_INET;
    server_addr.sin_addr.s_addr = INADDR_ANY;
    server_addr.sin_port = htons(PORT);
    
    // 绑定地址
    if (bind(server_fd, (struct sockaddr*)&server_addr, 
             sizeof(server_addr)) == -1) {
        perror("bind");
        close(server_fd);
        thread_pool_destroy(pool);
        exit(EXIT_FAILURE);
    }
    
    // 开始监听
    if (listen(server_fd, 5) == -1) {
        perror("listen");
        close(server_fd);
        thread_pool_destroy(pool);
        exit(EXIT_FAILURE);
    }
    
    printf("Thread Pool Server listening on port %d\n", PORT);
    
    while (1) {
        client_len = sizeof(client_addr);
        client_fd = accept(server_fd, (struct sockaddr*)&client_addr, 
                          &client_len);
        if (client_fd == -1) {
            perror("accept");
            continue;
        }
        
        // 添加任务到线程池
        if (add_task(client_fd, &client_addr) != 0) {
            fprintf(stderr, "Failed to add task to queue\n");
            close(client_fd);
        }
    }
    
    close(server_fd);
    thread_pool_destroy(pool);
    return 0;
}
```

#### 4.1.3 线程池模型优化

**动态线程池：**
根据任务队列长度动态调整线程数量：

```c
// 最小线程数
#define MIN_THREADS 5
// 最大线程数
#define MAX_THREADS 50
// 队列长度阈值
#define QUEUE_THRESHOLD 10

void adjust_thread_pool(thread_pool_t* pool) {
    int queue_size = get_queue_size(pool);
    
    if (queue_size > QUEUE_THRESHOLD && pool->thread_count < MAX_THREADS) {
        // 增加线程
        add_worker_thread(pool);
    } else if (queue_size == 0 && pool->thread_count > MIN_THREADS) {
        // 减少线程
        remove_worker_thread(pool);
    }
}
```

**任务优先级：**
为不同类型的任务设置优先级：

```c
typedef struct task {
    int client_fd;
    struct sockaddr_in client_addr;
    int priority;  // 优先级
    struct task* next;
} task_t;

// 按优先级插入任务
void add_task_priority(task_t* task) {
    task_t* current = pool->task_queue_head;
    task_t* prev = NULL;
    
    while (current != NULL && current->priority >= task->priority) {
        prev = current;
        current = current->next;
    }
    
    if (prev == NULL) {
        task->next = pool->task_queue_head;
        pool->task_queue_head = task;
    } else {
        task->next = prev->next;
        prev->next = task;
    }
}
```

#### 4.1.4 线程池模型性能分析

| 指标 | 值 | 说明 |
|------|-----|------|
| **并发连接数** | 高（> 10000） | 受限于线程池大小 |
| **吞吐量** | 高 | 避免线程创建开销 |
| **延迟** | 低 | 线程已就绪 |
| **资源占用** | 可控 | 固定线程数量 |
| **适用场景** | 高并发、I/O密集型 | Java线程池、Nginx worker进程 |

### 4.2 进程池模型

进程池模型（Process Pool / Prefork）预先创建一定数量的工作进程，主进程负责接受连接，然后通过进程间通信将连接分发给工作进程处理。

#### 4.2.1 进程池模型特点

**优点：**
- 避免频繁fork的开销
- 进程间隔离，稳定性好
- 可以充分利用多核CPU
- 适合CPU密集型任务

**缺点：**
- 进程间通信复杂
- 内存占用较高
- 进程切换开销较大

**适用场景：**
- 高并发场景
- CPU密集型任务
- 需要进程隔离的场景
- Apache prefork模式

#### 4.2.2 进程池模型实现

```c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
#include <sys/socket.h>
#include <netinet/in.h>
#include <arpa/inet.h>
#include <sys/wait.h>
#include <signal.h>
#include <errno.h>

#define PORT 8080
#define BUFFER_SIZE 1024
#define WORKER_PROCESSES 4
#define MAX_WORKERS 10

int server_fd;
int worker_count = 0;
pid_t workers[MAX_WORKERS];

// 信号处理：回收僵尸进程
void sigchld_handler(int sig) {
    pid_t pid;
    int status;
    
    while ((pid = waitpid(-1, &status, WNOHANG)) > 0) {
        printf("Worker process %d exited\n", pid);
        worker_count--;
        
        // 重启工作进程
        if (worker_count < WORKER_PROCESSES) {
            pid_t new_pid = fork();
            if (new_pid == 0) {
                // 子进程：处理连接
                worker_process();
                exit(0);
            } else if (new_pid > 0) {
                workers[worker_count++] = new_pid;
                printf("Restarted worker process %d\n", new_pid);
            }
        }
    }
}

void handle_client(int client_fd) {
    char buffer[BUFFER_SIZE];
    ssize_t bytes_received;
    
    printf("[PID %d] Handling client connection\n", getpid());
    
    while (1) {
        bytes_received = recv(client_fd, buffer, BUFFER_SIZE - 1, 0);
        if (bytes_received <= 0) {
            break;
        }
        
        buffer[bytes_received] = '\0';
        printf("[PID %d] Received: %s", getpid(), buffer);
        
        // 处理请求
        // 这里可以添加业务逻辑
        
        // 发送响应
        send(client_fd, buffer, bytes_received, 0);
    }
    
    close(client_fd);
    printf("[PID %d] Client disconnected\n", getpid());
}

// 工作进程函数
void worker_process() {
    int client_fd;
    struct sockaddr_in client_addr;
    socklen_t client_len;
    
    printf("[Worker PID %d] Started\n", getpid());
    
    while (1) {
        client_len = sizeof(client_addr);
        client_fd = accept(server_fd, (struct sockaddr*)&client_addr, 
                          &client_len);
        if (client_fd == -1) {
            if (errno == EINTR) {
                continue;
            }
            perror("accept");
            break;
        }
        
        printf("[Worker PID %d] Client connected from %s:%d\n",
               getpid(),
               inet_ntoa(client_addr.sin_addr),
               ntohs(client_addr.sin_port));
        
        handle_client(client_fd);
    }
}

// 创建进程池
void create_process_pool() {
    int i;
    pid_t pid;
    
    signal(SIGCHLD, sigchld_handler);
    
    for (i = 0; i < WORKER_PROCESSES; i++) {
        pid = fork();
        if (pid == 0) {
            // 子进程：处理连接
            worker_process();
            exit(0);
        } else if (pid > 0) {
            workers[worker_count++] = pid;
            printf("Created worker process %d\n", pid);
        } else {
            perror("fork");
            exit(EXIT_FAILURE);
        }
    }
}

int main() {
    struct sockaddr_in server_addr;
    
    // 创建套接字
    server_fd = socket(AF_INET, SOCK_STREAM, 0);
    if (server_fd == -1) {
        perror("socket");
        exit(EXIT_FAILURE);
    }
    
    // 设置地址重用
    int opt = 1;
    setsockopt(server_fd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt));
    
    // 配置服务器地址
    memset(&server_addr, 0, sizeof(server_addr));
    server_addr.sin_family = AF_INET;
    server_addr.sin_addr.s_addr = INADDR_ANY;
    server_addr.sin_port = htons(PORT);
    
    // 绑定地址
    if (bind(server_fd, (struct sockaddr*)&server_addr, 
             sizeof(server_addr)) == -1) {
        perror("bind");
        close(server_fd);
        exit(EXIT_FAILURE);
    }
    
    // 开始监听
    if (listen(server_fd, 5) == -1) {
        perror("listen");
        close(server_fd);
        exit(EXIT_FAILURE);
    }
    
    printf("Process Pool Server listening on port %d\n", PORT);
    printf("Main process PID: %d\n", getpid());
    
    // 创建进程池
    create_process_pool();
    
    // 主进程：等待所有工作进程
    while (worker_count > 0) {
        pause();
    }
    
    close(server_fd);
    return 0;
}
```

#### 4.2.3 进程池模型优化

**共享套接字（SO_REUSEPORT）：**
使用SO_REUSEPORT选项，让多个进程共享同一个端口：

```c
// 设置SO_REUSEPORT选项
int opt = 1;
setsockopt(server_fd, SOL_SOCKET, SO_REUSEPORT, &opt, sizeof(opt));

// 每个工作进程都绑定同一个端口
// 内核会自动进行负载均衡
```

**进程间通信优化：**
使用共享内存或消息队列进行进程间通信：

```c
#include <sys/ipc.h>
#include <sys/shm.h>
#include <sys/msg.h>

// 使用共享内存
int shm_id = shmget(IPC_PRIVATE, sizeof(shared_data), IPC_CREAT | 0666);
shared_data* data = (shared_data*)shmat(shm_id, NULL, 0);

// 使用消息队列
int msg_id = msgget(IPC_PRIVATE, IPC_CREAT | 0666);
msgsnd(msg_id, &message, sizeof(message), 0);
```

#### 4.2.4 进程池模型性能分析

| 指标 | 值 | 说明 |
|------|-----|------|
| **并发连接数** | 高（> 5000） | 受限于进程数量 |
| **吞吐量** | 高 | 避免fork开销 |
| **延迟** | 中等 | 进程切换开销 |
| **资源占用** | 高 | 每个进程独立内存空间 |
| **适用场景** | 高并发、CPU密集型 | Apache prefork、Nginx worker |

## 5. 事件驱动架构

### 5.1 事件驱动架构概述

事件驱动架构（Event-Driven Architecture）是高性能网络服务器的核心设计模式，通过事件循环和回调机制处理I/O操作，避免了线程/进程切换的开销。

#### 5.1.1 事件驱动架构特点

**优点：**
- 高并发能力（可处理数万连接）
- 低资源占用（单线程/少量线程）
- 低延迟（无线程切换）
- 适合I/O密集型应用

**缺点：**
- 不适合CPU密集型任务
- 编程复杂度较高
- 需要非阻塞I/O支持

**适用场景：**
- 高并发Web服务器
- 实时通信系统
- I/O密集型应用
- Node.js、Nginx、Redis等

#### 5.1.2 事件驱动架构分类

```
┌─────────────────────────────────────────────────────────────┐
│                  事件驱动架构分类                            │
├─────────────────────────────────────────────────────────────┤
│  🔄 Reactor模式                                              │
│  ├── 单Reactor单线程                                         │
│  ├── 单Reactor多线程                                         │
│  └── 多Reactor多线程                                         │
├─────────────────────────────────────────────────────────────┤
│  ⚡ Proactor模式                                             │
│  ├── 异步I/O完成                                             │
│  ├── 操作系统完成I/O操作                                     │
│  └── 回调通知应用层                                          │
└─────────────────────────────────────────────────────────────┘
```

### 5.2 Reactor模式

Reactor模式是事件驱动架构的核心，通过事件分发器（Event Demultiplexer）监听多个文件描述符，当I/O事件就绪时，通知相应的处理器进行处理。

#### 5.2.1 Reactor模式组件

**Reactor模式包含以下组件：**

1. **Event Demultiplexer（事件分发器）**：
   - select/poll/epoll等I/O多路复用机制
   - 监听文件描述符的I/O事件

2. **Event Handler（事件处理器）**：
   - 处理具体的I/O事件
   - 包含回调函数

3. **Reactor（反应器）**：
   - 事件循环
   - 分发事件到相应的处理器

4. **Acceptor（接受器）**：
   - 处理新连接事件
   - 创建新的连接处理器

#### 5.2.2 单Reactor单线程模型

**架构图：**
```
┌─────────────────────────────────────────────────────────────┐
│                     单Reactor单线程                          │
├─────────────────────────────────────────────────────────────┤
│  Main Thread                                                 │
│  ┌─────────────────────────────────────────────────────┐    │
│  │  Reactor (Event Loop)                               │    │
│  │  ├── epoll_wait()                                   │    │
│  │  ├── 事件分发                                        │    │
│  │  └── 调用Handler                                     │    │
│  └─────────────────────────────────────────────────────┘    │
│         │              │              │                      │
│         ▼              ▼              ▼                      │
│  ┌──────────┐   ┌──────────┐   ┌──────────┐                 │
│  │ Acceptor │   │ Handler1 │   │ Handler2 │                 │
│  └──────────┘   └──────────┘   └──────────┘                 │
└─────────────────────────────────────────────────────────────┘
```

**实现示例：**
```c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
#include <sys/epoll.h>
#include <sys/socket.h>
#include <netinet/in.h>
#include <arpa/inet.h>
#include <fcntl.h>

#define PORT 8080
#define BUFFER_SIZE 1024
#define MAX_EVENTS 1000

// 事件处理器接口
typedef struct event_handler {
    int fd;
    void (*handle_read)(struct event_handler* self);
    void (*handle_write)(struct event_handler* self);
    void (*handle_error)(struct event_handler* self);
} event_handler_t;

// Acceptor处理器
typedef struct {
    event_handler_t base;
    int server_fd;
} acceptor_t;

// Connection处理器
typedef struct {
    event_handler_t base;
    char buffer[BUFFER_SIZE];
    int buffer_len;
} connection_t;

// Reactor结构
typedef struct {
    int epoll_fd;
    struct epoll_event events[MAX_EVENTS];
    event_handler_t* handlers[MAX_EVENTS];
    int handler_count;
} reactor_t;

reactor_t reactor;

// Acceptor处理新连接
void acceptor_handle_read(event_handler_t* handler) {
    acceptor_t* acceptor = (acceptor_t*)handler;
    struct sockaddr_in client_addr;
    socklen_t client_len = sizeof(client_addr);
    int client_fd;
    connection_t* conn;
    struct epoll_event event;
    
    client_fd = accept(acceptor->server_fd, 
                      (struct sockaddr*)&client_addr, &client_len);
    if (client_fd == -1) {
        perror("accept");
        return;
    }
    
    // 设置非阻塞
    int flags = fcntl(client_fd, F_GETFL, 0);
    fcntl(client_fd, F_SETFL, flags | O_NONBLOCK);
    
    // 创建连接处理器
    conn = malloc(sizeof(connection_t));
    memset(conn, 0, sizeof(connection_t));
    conn->base.fd = client_fd;
    conn->base.handle_read = connection_handle_read;
    conn->base.handle_write = connection_handle_write;
    conn->base.handle_error = connection_handle_error;
    
    // 添加到epoll
    event.events = EPOLLIN | EPOLLET;
    event.data.ptr = &conn->base;
    epoll_ctl(reactor.epoll_fd, EPOLL_CTL_ADD, client_fd, &event);
    
    printf("New client connected: %s:%d\n",
           inet_ntoa(client_addr.sin_addr),
           ntohs(client_addr.sin_port));
}

// Connection处理读事件
void connection_handle_read(event_handler_t* handler) {
    connection_t* conn = (connection_t*)handler;
    ssize_t bytes_received;
    
    bytes_received = recv(conn->base.fd, conn->buffer, BUFFER_SIZE - 1, 0);
    if (bytes_received > 0) {
        conn->buffer[bytes_received] = '\0';
        conn->buffer_len = bytes_received;
        printf("Received: %s", conn->buffer);
        
        // 回显数据
        send(conn->base.fd, conn->buffer, bytes_received, 0);
    } else if (bytes_received == 0) {
        printf("Client disconnected\n");
        connection_handle_error(handler);
    } else {
        if (errno != EAGAIN && errno != EWOULDBLOCK) {
            perror("recv");
            connection_handle_error(handler);
        }
    }
}

// Connection处理写事件
void connection_handle_write(event_handler_t* handler) {
    // 写事件处理
    connection_t* conn = (connection_t*)handler;
    // 实现写逻辑
}

// Connection处理错误
void connection_handle_error(event_handler_t* handler) {
    connection_t* conn = (connection_t*)handler;
    epoll_ctl(reactor.epoll_fd, EPOLL_CTL_DEL, conn->base.fd, NULL);
    close(conn->base.fd);
    free(conn);
}

// Reactor事件循环
void reactor_run() {
    int nfds, i;
    event_handler_t* handler;
    
    while (1) {
        nfds = epoll_wait(reactor.epoll_fd, reactor.events, MAX_EVENTS, -1);
        if (nfds == -1) {
            perror("epoll_wait");
            break;
        }
        
        for (i = 0; i < nfds; i++) {
            handler = (event_handler_t*)reactor.events[i].data.ptr;
            
            if (reactor.events[i].events & EPOLLIN) {
                handler->handle_read(handler);
            }
            if (reactor.events[i].events & EPOLLOUT) {
                handler->handle_write(handler);
            }
            if (reactor.events[i].events & (EPOLLERR | EPOLLHUP)) {
                handler->handle_error(handler);
            }
        }
    }
}

int main() {
    int server_fd;
    struct sockaddr_in server_addr;
    struct epoll_event event;
    acceptor_t* acceptor;
    
    // 创建epoll
    reactor.epoll_fd = epoll_create1(0);
    if (reactor.epoll_fd == -1) {
        perror("epoll_create1");
        exit(EXIT_FAILURE);
    }
    
    // 创建服务器套接字
    server_fd = socket(AF_INET, SOCK_STREAM, 0);
    if (server_fd == -1) {
        perror("socket");
        exit(EXIT_FAILURE);
    }
    
    // 设置非阻塞
    int flags = fcntl(server_fd, F_GETFL, 0);
    fcntl(server_fd, F_SETFL, flags | O_NONBLOCK);
    
    // 设置地址重用
    int opt = 1;
    setsockopt(server_fd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt));
    
    // 配置服务器地址
    memset(&server_addr, 0, sizeof(server_addr));
    server_addr.sin_family = AF_INET;
    server_addr.sin_addr.s_addr = INADDR_ANY;
    server_addr.sin_port = htons(PORT);
    
    // 绑定和监听
    if (bind(server_fd, (struct sockaddr*)&server_addr, 
             sizeof(server_addr)) == -1) {
        perror("bind");
        exit(EXIT_FAILURE);
    }
    
    if (listen(server_fd, 5) == -1) {
        perror("listen");
        exit(EXIT_FAILURE);
    }
    
    // 创建Acceptor
    acceptor = malloc(sizeof(acceptor_t));
    memset(acceptor, 0, sizeof(acceptor_t));
    acceptor->base.fd = server_fd;
    acceptor->base.handle_read = acceptor_handle_read;
    acceptor->server_fd = server_fd;
    
    // 添加服务器套接字到epoll
    event.events = EPOLLIN;
    event.data.ptr = &acceptor->base;
    epoll_ctl(reactor.epoll_fd, EPOLL_CTL_ADD, server_fd, &event);
    
    printf("Single Reactor Single Thread Server listening on port %d\n", PORT);
    
    // 运行事件循环
    reactor_run();
    
    close(server_fd);
    close(reactor.epoll_fd);
    return 0;
}
```

#### 5.2.3 单Reactor多线程模型

**架构图：**
```
┌─────────────────────────────────────────────────────────────┐
│                     单Reactor多线程                          │
├─────────────────────────────────────────────────────────────┤
│  Main Thread (Reactor)                                      │
│  ┌─────────────────────────────────────────────────────┐    │
│  │  Event Loop                                         │    │
│  │  ├── 接受连接                                        │    │
│  │  └── 分发I/O事件到线程池                             │    │
│  └─────────────────────────────────────────────────────┘    │
│         │                                                    │
│         ▼                                                    │
│  ┌─────────────────────────────────────────────────────┐    │
│  │  Thread Pool                                         │    │
│  │  ├── Worker Thread 1                                 │    │
│  │  ├── Worker Thread 2                                 │    │
│  │  └── Worker Thread N                                 │    │
│  └─────────────────────────────────────────────────────┘    │
└─────────────────────────────────────────────────────────────┘
```

**特点：**
- Reactor线程负责接受连接和I/O事件分发
- 工作线程池处理业务逻辑
- 适合CPU密集型任务

#### 5.2.4 多Reactor多线程模型

**架构图：**
```
┌─────────────────────────────────────────────────────────────┐
│                     多Reactor多线程                          │
├─────────────────────────────────────────────────────────────┤
│  Main Reactor (Main Thread)                                 │
│  ┌─────────────────────────────────────────────────────┐    │
│  │  接受新连接，分发给Sub Reactor                        │    │
│  └─────────────────────────────────────────────────────┘    │
│         │                                                    │
│         ├──────────────┬──────────────┬──────────────┐      │
│         ▼              ▼              ▼              ▼      │
│  ┌──────────┐   ┌──────────┐   ┌──────────┐   ┌──────────┐│
│  │Sub Reactor│   │Sub Reactor│   │Sub Reactor│   │Sub Reactor││
│  │(Thread 1) │   │(Thread 2) │   │(Thread 3) │   │(Thread N) ││
│  └──────────┘   └──────────┘   └──────────┘   └──────────┘│
└─────────────────────────────────────────────────────────────┘
```

**特点：**
- Main Reactor负责接受连接
- Sub Reactor负责处理I/O事件
- 每个Sub Reactor运行在独立线程中
- 适合高并发场景（Netty、Muduo等）

#### 5.2.5 Reactor模式性能分析

| 模型 | 并发连接数 | 吞吐量 | 延迟 | 适用场景 |
|------|-----------|--------|------|----------|
| **单Reactor单线程** | 高（> 10000） | 高 | 低 | I/O密集型、Redis |
| **单Reactor多线程** | 高（> 10000） | 高 | 低 | I/O+CPU混合 |
| **多Reactor多线程** | 极高（> 50000） | 极高 | 极低 | 高并发、Netty |

### 5.3 Proactor模式

Proactor模式是另一种事件驱动架构，与Reactor模式的主要区别在于：Proactor模式中，应用程序发起异步I/O操作，操作系统完成I/O操作后通知应用程序。

#### 5.3.1 Proactor模式特点

**与Reactor模式对比：**

| 特性 | Reactor模式 | Proactor模式 |
|------|------------|-------------|
| **I/O操作** | 应用程序完成 | 操作系统完成 |
| **通知时机** | I/O就绪时通知 | I/O完成时通知 |
| **编程复杂度** | 中等 | 较高 |
| **性能** | 高 | 极高 |
| **实现方式** | epoll/select | io_uring/aio |

**优点：**
- 更高的性能（零拷贝、异步I/O）
- 减少系统调用次数
- 更好的CPU利用率

**缺点：**
- 编程复杂度高
- 需要操作系统支持（Linux io_uring）
- 调试困难

#### 5.3.2 Proactor模式实现（io_uring）

```c
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
#include <sys/socket.h>
#include <netinet/in.h>
#include <arpa/inet.h>
#include <liburing.h>
#include <fcntl.h>

#define PORT 8080
#define BUFFER_SIZE 1024
#define QUEUE_DEPTH 256

// 连接信息
typedef struct {
    int fd;
    char buffer[BUFFER_SIZE];
    int buffer_len;
} connection_t;

// 初始化io_uring
int setup_uring(struct io_uring* ring) {
    int ret = io_uring_queue_init(QUEUE_DEPTH, ring, 0);
    if (ret < 0) {
        fprintf(stderr, "io_uring_queue_init failed: %s\n", strerror(-ret));
        return -1;
    }
    return 0;
}

// 提交accept请求
void submit_accept(struct io_uring* ring, int server_fd) {
    struct io_uring_sqe* sqe = io_uring_get_sqe(ring);
    io_uring_prep_accept(sqe, server_fd, NULL, NULL, 0);
    
    connection_t* conn = malloc(sizeof(connection_t));
    io_uring_sqe_set_data(sqe, conn);
    
    io_uring_submit(ring);
}

// 提交recv请求
void submit_recv(struct io_uring* ring, connection_t* conn) {
    struct io_uring_sqe* sqe = io_uring_get_sqe(ring);
    io_uring_prep_recv(sqe, conn->fd, conn->buffer, BUFFER_SIZE, 0);
    io_uring_sqe_set_data(sqe, conn);
    io_uring_submit(ring);
}

// 提交send请求
void submit_send(struct io_uring* ring, connection_t* conn, int len) {
    struct io_uring_sqe* sqe = io_uring_get_sqe(ring);
    io_uring_prep_send(sqe, conn->fd, conn->buffer, len, 0);
    io_uring_sqe_set_data(sqe, conn);
    io_uring_submit(ring);
}

int main() {
    struct io_uring ring;
    struct io_uring_cqe* cqe;
    int server_fd;
    struct sockaddr_in server_addr;
    connection_t* conn;
    int ret;
    
    // 初始化io_uring
    if (setup_uring(&ring) < 0) {
        exit(EXIT_FAILURE);
    }
    
    // 创建服务器套接字
    server_fd = socket(AF_INET, SOCK_STREAM, 0);
    if (server_fd == -1) {
        perror("socket");
        exit(EXIT_FAILURE);
    }
    
    // 设置非阻塞
    int flags = fcntl(server_fd, F_GETFL, 0);
    fcntl(server_fd, F_SETFL, flags | O_NONBLOCK);
    
    // 设置地址重用
    int opt = 1;
    setsockopt(server_fd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt));
    
    // 配置服务器地址
    memset(&server_addr, 0, sizeof(server_addr));
    server_addr.sin_family = AF_INET;
    server_addr.sin_addr.s_addr = INADDR_ANY;
    server_addr.sin_port = htons(PORT);
    
    // 绑定和监听
    if (bind(server_fd, (struct sockaddr*)&server_addr, 
             sizeof(server_addr)) == -1) {
        perror("bind");
        exit(EXIT_FAILURE);
    }
    
    if (listen(server_fd, 5) == -1) {
        perror("listen");
        exit(EXIT_FAILURE);
    }
    
    printf("Proactor Server (io_uring) listening on port %d\n", PORT);
    
    // 提交初始accept请求
    submit_accept(&ring, server_fd);
    
    // 事件循环
    while (1) {
        // 等待完成事件
        ret = io_uring_wait_cqe(&ring, &cqe);
        if (ret < 0) {
            fprintf(stderr, "io_uring_wait_cqe failed\n");
            break;
        }
        
        conn = (connection_t*)io_uring_cqe_get_data(cqe);
        
        if (cqe->res < 0) {
            fprintf(stderr, "Async operation failed: %s\n", strerror(-cqe->res));
            if (conn->fd != server_fd) {
                close(conn->fd);
                free(conn);
            }
        } else if (conn->fd == server_fd) {
            // 新连接
            int client_fd = cqe->res;
            printf("New client connected: fd=%d\n", client_fd);
            
            // 设置非阻塞
            flags = fcntl(client_fd, F_GETFL, 0);
            fcntl(client_fd, F_SETFL, flags | O_NONBLOCK);
            
            // 创建连接对象
            conn = malloc(sizeof(connection_t));
            memset(conn, 0, sizeof(connection_t));
            conn->fd = client_fd;
            
            // 提交recv请求
            submit_recv(&ring, conn);
            
            // 继续接受新连接
            submit_accept(&ring, server_fd);
        } else {
            // I/O操作完成
            if (cqe->res > 0) {
                // 数据接收完成
                conn->buffer_len = cqe->res;
                conn->buffer[conn->buffer_len] = '\0';
                printf("Received: %s", conn->buffer);
                
                // 提交send请求
                submit_send(&ring, conn, conn->buffer_len);
            } else if (cqe->res == 0) {
                // 连接关闭
                printf("Client disconnected\n");
                close(conn->fd);
                free(conn);
            } else {
                // 发送完成，继续接收
                submit_recv(&ring, conn);
            }
        }
        
        io_uring_cqe_seen(&ring, cqe);
    }
    
    close(server_fd);
    io_uring_queue_exit(&ring);
    return 0;
}
```

#### 5.3.3 Proactor模式性能分析

| 指标 | 值 | 说明 |
|------|-----|------|
| **并发连接数** | 极高（> 100000） | 受限于系统资源 |
| **吞吐量** | 极高 | 零拷贝、异步I/O |
| **延迟** | 极低 | 减少系统调用 |
| **资源占用** | 低 | 单线程/少量线程 |
| **适用场景** | 超高并发、高性能 | io_uring、Windows IOCP |

## 6. 负载均衡与代理

### 6.1 负载均衡概述

负载均衡（Load Balancing）是将网络请求分发到多个服务器上，以提高系统的可用性、扩展性和性能。

#### 6.1.1 负载均衡分类

**按层次分类：**

1. **DNS负载均衡**：
   - 在DNS层面进行负载均衡
   - 返回不同的IP地址
   - 简单但不够灵活

2. **四层负载均衡（L4）**：
   - 基于IP和端口
   - TCP/UDP协议
   - 性能高，但功能有限

3. **七层负载均衡（L7）**：
   - 基于HTTP/HTTPS协议
   - 可以基于URL、Header等
   - 功能强大，但性能较低

**按实现方式分类：**

1. **硬件负载均衡**：
   - F5、A10等专用设备
   - 性能极高，但成本高

2. **软件负载均衡**：
   - Nginx、HAProxy、LVS
   - 成本低，配置灵活

### 6.2 负载均衡算法

#### 6.2.1 轮询（Round Robin）

**原理：**
按顺序将请求分发到各个服务器。

**实现：**
```c
typedef struct {
    char* server_addr;
    int port;
    int weight;
    int current_connections;
} backend_server_t;

backend_server_t servers[] = {
    {"192.168.1.10", 8080, 1, 0},
    {"192.168.1.11", 8080, 1, 0},
    {"192.168.1.12", 8080, 1, 0},
};

int current_index = 0;

backend_server_t* round_robin_select() {
    backend_server_t* server = &servers[current_index];
    current_index = (current_index + 1) % 3;
    return server;
}
```

#### 6.2.2 加权轮询（Weighted Round Robin）

**原理：**
根据服务器权重进行轮询，权重高的服务器获得更多请求。

**实现：**
```c
int total_weight = 0;
int current_weight = 0;

backend_server_t* weighted_round_robin_select() {
    int max_weight = -1;
    int selected_index = 0;
    
    // 计算总权重
    for (int i = 0; i < 3; i++) {
        total_weight += servers[i].weight;
        int weight = servers[i].weight + servers[i].current_connections;
        if (weight > max_weight) {
            max_weight = weight;
            selected_index = i;
        }
    }
    
    servers[selected_index].current_connections++;
    return &servers[selected_index];
}
```

#### 6.2.3 最少连接（Least Connections）

**原理：**
将请求分发到当前连接数最少的服务器。

**实现：**
```c
backend_server_t* least_connections_select() {
    int min_connections = INT_MAX;
    int selected_index = 0;
    
    for (int i = 0; i < 3; i++) {
        if (servers[i].current_connections < min_connections) {
            min_connections = servers[i].current_connections;
            selected_index = i;
        }
    }
    
    servers[selected_index].current_connections++;
    return &servers[selected_index];
}
```

#### 6.2.4 IP哈希（IP Hash）

**原理：**
根据客户端IP地址计算哈希值，将同一IP的请求分发到同一服务器。

**实现：**
```c
unsigned int hash_ip(const char* ip) {
    unsigned int hash = 0;
    for (int i = 0; ip[i] != '\0'; i++) {
        hash = hash * 31 + ip[i];
    }
    return hash;
}

backend_server_t* ip_hash_select(const char* client_ip) {
    unsigned int hash = hash_ip(client_ip);
    int selected_index = hash % 3;
    return &servers[selected_index];
}
```

### 6.3 反向代理

反向代理（Reverse Proxy）是负载均衡的一种实现方式，代理服务器接收客户端请求，然后转发到后端服务器。

#### 6.3.1 反向代理架构

```
┌──────────┐      ┌──────────────┐      ┌──────────┐
│ Client 1 │─────▶│              │─────▶│ Server 1 │
└──────────┘      │              │      └──────────┘
                  │ Reverse Proxy│
┌──────────┐      │   (Nginx)    │      ┌──────────┐
│ Client 2 │─────▶│              │─────▶│ Server 2 │
└──────────┘      │              │      └──────────┐
                  └──────────────┘                │
┌──────────┐                                      │
│ Client 3 │─────────────────────────────────────┘
└──────────┘
```

#### 6.3.2 反向代理实现要点

**连接管理：**
- 保持连接池
- 复用后端连接
- 超时处理

**健康检查：**
- 定期检查后端服务器状态
- 自动剔除故障服务器
- 自动恢复可用服务器

**SSL终止：**
- 在代理服务器上终止SSL
- 减轻后端服务器负担
- 统一证书管理

### 6.4 服务器架构选择建议

#### 6.4.1 架构选择决策树

```
开始
  │
  ├─ 并发连接数 < 100?
  │   └─ 是 → 串行服务器
  │   └─ 否 ↓
  │
  ├─ 并发连接数 < 1000?
  │   └─ 是 → 进程/线程每连接模型
  │   └─ 否 ↓
  │
  ├─ 任务类型?
  │   ├─ CPU密集型 → 进程池/线程池模型
  │   └─ I/O密集型 ↓
  │
  ├─ 并发连接数 < 10000?
  │   └─ 是 → 单Reactor单线程
  │   └─ 否 ↓
  │
  ├─ 需要CPU处理?
  │   └─ 是 → 单Reactor多线程
  │   └─ 否 ↓
  │
  └─ 超高并发?
      └─ 是 → 多Reactor多线程 / Proactor模式
```

#### 6.4.2 实际应用案例

| 应用 | 架构模型 | 说明 |
|------|---------|------|
| **Redis** | 单Reactor单线程 | I/O密集型，单线程处理所有请求 |
| **Nginx** | 多进程 + epoll | 每个worker进程一个事件循环 |
| **Netty** | 多Reactor多线程 | Main Reactor + Sub Reactor |
| **Apache** | 进程池/线程池 | Prefork/Worker模式 |
| **Node.js** | 单Reactor单线程 + 线程池 | 事件循环 + libuv线程池 |

## 7. 总结

### 7.1 服务器架构演进

从简单的串行服务器到高性能的事件驱动架构，服务器架构经历了以下演进：

1. **串行服务器**：最简单的模型，适合学习
2. **进程/线程每连接**：中等并发，资源消耗大
3. **进程池/线程池**：高并发，资源可控
4. **事件驱动架构**：超高并发，资源占用低

### 7.2 架构选择原则

1. **根据并发量选择**：低并发用简单模型，高并发用事件驱动
2. **根据任务类型选择**：CPU密集型用多进程/多线程，I/O密集型用事件驱动
3. **根据资源限制选择**：内存充足用进程池，内存受限用线程池或事件驱动
4. **根据开发复杂度选择**：简单应用用线程池，高性能应用用事件驱动

### 7.3 性能优化建议

1. **使用连接池**：减少连接建立开销
2. **使用非阻塞I/O**：提高并发能力
3. **使用I/O多路复用**：epoll/kqueue等
4. **使用异步I/O**：io_uring等现代接口
5. **使用负载均衡**：水平扩展系统能力
6. **使用缓存**：减少后端压力

### 7.4 最佳实践

1. **监控和日志**：完善的监控和日志系统
2. **优雅关闭**：正确处理信号和资源清理
3. **错误处理**：完善的错误处理和恢复机制
4. **安全考虑**：输入验证、权限控制、DDoS防护
5. **性能测试**：定期进行压力测试和性能调优

---

> **💡 提示**：服务器架构设计是一个复杂的主题，需要根据实际应用场景选择合适的架构。建议从简单的模型开始，逐步优化和演进，最终达到性能和复杂度的平衡。

**文档版本**：v1.0  
**创建时间**：2025年12月06日 17:04:30 CST  
**最后更新**：2025年12月10日 23:04:30 CST  
**维护者**：Gamma
